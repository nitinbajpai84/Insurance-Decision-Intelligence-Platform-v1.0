            ...
            <section className="rounded-lg border border-gray-100 bg-white p-5 shadow-sm">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={FUNNEL_STEPS.map((s) => ({ step: s, value: data.funnel[s] || 0 }))} layout="vertical" margin={{ left: 20, right: 50 }}>
                  ...
                  <Bar dataKey="value" fill="#D04A02" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </section>
            ...
            <section className="rounded-lg border border-gray-100 bg-white p-5 shadow-sm">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={data.by_channel} margin={{ left: 10, right: 10 }}>
                  ...
                  <Tooltip />
                  <Bar dataKey="roi" fill="#EB8C00" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </section>
          </div>
                ...
                <tbody>
                  {data.roi_leaderboard.map((c, i) => (
                    <tr key={i} className="border-t border-gray-100">
                      ...
                      <td className="px-3 py-2 font-bold text-pwc-orange">{c.roi_multiple ?? "—"}x</td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  return (
    <article className="rounded-lg border border-gray-100 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between">
        ...
        <Link href={askWhyHref(q, { process_id: "campaign_effectiveness", page: "campaign-process", role: "Campaign Manager" })} className="text-gray-300 hover:text-pwc-orange"><HelpCircle size={15} /></Link>
      </div>
